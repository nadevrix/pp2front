...
export default function NewProjectPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/dashboard/projects/${res.project.id}?created=1`);
    } catch (e: any) {
      setError(e.message || 'Error creando el proyecto');
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-10">
      ...
      <h1 className="text-3xl font-bold tracking-tight mb-2">Nuevo proyecto</h1>
      <p className="text-slate-400 mb-8">
        Cada proyecto tiene su propia API key y wallet de payout (donde recibís el USDC).
      </p>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8">
        <form onSubmit={onSubmit} className="space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">Razón / descripción</label>
            <input
              type="text"
              value={reason}
              onChange={e => setReason(e.target.value)}
              placeholder="Ej: Cobro de productos digitales"
              required
              className="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
            />
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">
              Payout wallet
              <span className="ml-1.5 text-slate-500 font-normal text-xs">— Stellar public key donde vas a recibir los pagos</span>
            </label>
            <input
              type="text"
              value={payoutWallet}
